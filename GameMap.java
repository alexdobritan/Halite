...
public class GameMap{
	...
	public ArrayList<Location> getLocationsWithinDistance(Location center, int distance) {
		ArrayList<Location> locations = new ArrayList<>();
		
		int startX = Math.max(0, center.getX() - distance);
		int endX = Math.min(width - 1, center.getX() + distance);
		int startY = Math.max(0, center.getY() - distance);
		int endY = Math.min(height - 1, center.getY() + distance);
		
		for (int y = startY; y <= endY; y++) {
			for (int x = startX; x <= endX; x++) {
				Location location = getLocation(x, y);
				if (getDistance(center, location) <= distance) {
